...
// 2020-09-11 21:34:57.809895 +0200 CEST m=+0.014134936
...
// from the pool.
func (p *Pool) GetInt64(length int, bd BitDepth) Signed {
	if p == nil {
		return nil
	}
	return Int64{
		buffer:   p.i64.Get().([]int64)[:p.channels*length],
...
func (p *Pool) PutInt64(s Signed) {
	...
	if sig, ok := s.(Int64); ok {
		buf := sig.buffer[:sig.Cap()]
		for i := range buf {
			buf[i] = 0
		}
		p.i64.Put(sig.buffer)
	} else {
