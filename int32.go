...
// 2020-12-16 20:52:36.100456 +0100 CET m=+0.020231761
...
// i32 is int32 Signed fixed-point signal.
type i32 struct {
	buffer []int32
	channels
	bitDepth
...
// Int32 selects a new sequential int32 signal buffer.
// from the pool.
...
func (s *i32) AppendSample(value int64) {
...
// Sample values are capped by maximum value of the buffer bit depth.
func (s i32) SetSample(i int, value int64) {
	s.buffer[i] = int32(s.BitDepth().SignedValue(value))
}

// Puti32 places signal buffer back to the pool. If a type of
// provided buffer isn't i32 or its capacity doesn't equal
// allocator capacity, the function will panic.
func (s *i32) Free(p *PoolAllocator) {
...
func int32pool(size int) sync.Pool {
	return sync.Pool{
		New: func() interface{} {
			return &i32{
				buffer: make([]int32, 0, size),
			}
		},
	}
}
...
func (s *i32) Slice(start, end int) Signed {
...
func (s *i32) Channel(c int) Signed {
