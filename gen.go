...
func main() {
	...
	for _, st := range signedTypes {
		generateSigned(st)
	}
}

func generateSigned(st signedType) {
	f, err := os.Create(fmt.Sprintf("%s.go", st.Builtin))
	die("create file", err)
	defer f.Close()

	err = signedTemplate.Execute(f, st)
	die("execute template", err)
}
