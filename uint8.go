...
// 2020-05-17 19:45:45.309264 +0200 CEST m=+0.012074453
package signal
...
// Uint8 allocates new sequential uint8 signal buffer.
...
func (s Uint8) Append(src Unsigned) Unsigned {
	...
	if s.Cap() < s.Len()+src.Len() {
		...
		s.buffer = append(s.buffer[:s.Cap()], make([]uint8, src.Cap())...)[:s.Len()]
	}
	result := Unsigned(s)
