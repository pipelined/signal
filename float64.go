...
// 2020-12-16 20:01:52.995129 +0100 CET m=+0.021021639
...
// f64 is float64 floating-point signal.
type f64 struct {
	buffer []float64
	channels
}
...
// from the pool.
func (p *PoolAllocator) GetFloat64() Floating {
	s := p.f64.Get().(*f64)
...
func (s *f64) AppendSample(value float64) {
...
// Putf64 places signal buffer back to the pool. If a type of
// provided buffer isn't f64 or its capacity doesn't equal
...
func float64pool(size int) sync.Pool {
	return sync.Pool{
		New: func() interface{} {
			return &f64{
...
func ReadStripedFloat64(src Floating, dst [][]float64) (read int) {
	...
	for c := 0; c < src.Channels(); c++ {
		...
		for i := 0; i < length; i++ {
			dst[c][i] = float64(src.Sample(src.BufferIndex(c, i)))
		}
	}
	return
}
