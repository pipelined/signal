...
// 2020-12-16 20:52:36.091648 +0100 CET m=+0.011424202
...
// i8 is int8 Signed fixed-point signal.
type i8 struct {
	buffer []int8
	channels
	bitDepth
...
// Int8 selects a new sequential int8 signal buffer.
// from the pool.
...
func (s *i8) AppendSample(value int64) {
...
// Sample values are capped by maximum value of the buffer bit depth.
func (s i8) SetSample(i int, value int64) {
	s.buffer[i] = int8(s.BitDepth().SignedValue(value))
}

// Puti8 places signal buffer back to the pool. If a type of
// provided buffer isn't i8 or its capacity doesn't equal
// allocator capacity, the function will panic.
func (s *i8) Free(p *PoolAllocator) {
...
func (s *i8) Slice(start, end int) Signed {
...
func (s *i8) Channel(c int) Signed {
