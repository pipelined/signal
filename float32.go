...
// 2020-12-16 20:52:36.090368 +0100 CET m=+0.010144469
...
// f32 is float32 floating-point signal.
type f32 struct {
	buffer []float32
	channels
}
...
// Float32 selects a new sequential float32 signal buffer.
// from the pool.
...
func (s *f32) AppendSample(value float64) {
...
func (s *f32) SetSample(i int, value float64) {
	s.buffer[i] = float32(value)
}

// Putf32 places signal buffer back to the pool. If a type of
// provided buffer isn't f32 or its capacity doesn't equal
// allocator capacity, the function will panic.
func (s *f32) Free(p *PoolAllocator) {
...
func (s *f32) Sample(i int) float64 {
...
func (s *f32) Append(src Floating) {
...
func (s *f32) Slice(start, end int) Floating {
...
func (s *f32) Channel(c int) Floating {
